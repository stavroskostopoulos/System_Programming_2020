...
int main(int argc, char *argv[]){
    ...
    printf("\n\n%d %d %s\n\n", workernumber, buffersize, input_dir);

    int worker_fd=0;//The fd of this worker (Aggregator to worker communication)

    char message[buffersize];//Our buffer for the read system calls
    ...
    int worker2agg_fd=0;//The fd of this worker ( worker to Aggregator communication)



    int firsttime=0;//A flag to let us know when we received a new folder name that we should manage | Some thing(such as mallocs) have to be done only once
    int total_records=0;
    ...
    //Generate the diseaseAggregator to Worker pipename
    ...
    for(;;){
        ...
        //Diavase apo to pipe an sou exei anate8ei directory
        if(read(worker_fd, message , buffersize) < 0){
            perror("problem  in  reading");
            exit(5);
        }else{//No problem in reading

            char* msg;//the final pipe messagethat we read will be kept here
            char* msg_temp;//Wil help us get the char* msg correctly


            //printf("\n\nasdfsdfsadf %s\n\n", message);
            msg_temp = read_message(worker_fd, message, buffersize);
            msg = (char*)malloc( (strlen(msg_temp)+1)*sizeof(char) );
            strcpy(msg, msg_temp);
            free(msg_temp);
            if(msg[0]!='/'){//if it NOT a query




                //printf("\nMessage Received to worker number %d:%skeno%zu\n", workernumber, msg, strlen(msg));

/* Diaxeirismos twn files tou folder pou molis anatethike ston worker*/

                //Message(==msg) == folder name pou anate8hke se auton ton worker
                //ara anoigoume to folder me onoma message

                //Create the path
                path = (char*)malloc( (strlen(/*argv[2]*/input_dir)+1+strlen(msg)+1)*sizeof(char) );//argv[2]->input_dir
                sprintf(path, "%s/%s", /*argv[2]*/input_dir, msg);
...
/* Open the files and add the data to the data structures */
                    for(j=0 ; j<filecount ; j++){//Take one by one the file names sorted in the date array
                        ...
                        path = (char*)malloc( (strlen(/*argv[2]*/input_dir)+1+strlen(msg)+1+strlen(date[j])+1)*sizeof(char) );//argv[2]->input_dir
                        sprintf(path, "%s/%s/%s", /*argv[2]*/input_dir, msg, date[j]);
                        ...
                        while(fscanf(file, "%s %s %s %s %s %s\n", recordID, status, patientFirstName, patientLastName, diseaseID, age)!=EOF){//Get the records from the patientfile until you get EOF
                            ...
                            if(i==0){//For the first record create the first node
                                ...
                                if( firsttime==0 ){//If it is the first time(The first file we are handling)
                                    root = createListNode(recordID, patientFirstName, patientLastName, diseaseID, msg, date[j], age, status);//message->country name | date[j]->date (will figure out in the function if it is the ENTRY or EXIT date)
                                    ...
                                    temproot = createListNode(recordID, patientFirstName, patientLastName, diseaseID, msg, date[j], age, status);//message->country name | date[j]->date (will figure out in the function if it is the ENTRY or EXIT date)
                                ...
                                if(!strcmp(status, "ENTER")){
                                    ...
                                    if( (check_if_bucketnode_with_this_name_exists( recordID, recordhash, hash_number, BUCKETSIZE)==1) ){//If there is a recordID equal tothis one
                                        ...
                                        temp = createListNode(recordID, patientFirstName, patientLastName, diseaseID, msg, date[j], age, status);
...
/* Now we get the stats for the specific date (date[j]) for the specific country(message),and we traverse through the diseases (diseasehash)*/
                    ...
                    //China
                    char* introduction;
                    introduction = (char*)malloc( (strlen(date[j])+1+strlen(msg)+1+1)*sizeof(char) );
                    ...
                    tempstring = (char*)malloc( (4000)*sizeof(char) );
                        ...
                        while(tempo!=NULL){//Pairnw ena ena ta buckets
                            for(z=0 ; z<(BUCKETSIZE/(sizeof(Bucket_node))) ; z++){//Pairnw ena ena ta bucket nodes(dhladh ta diseases)
                                ...
                                if(tempo->bucket_node_array[z]!=NULL){
                                    ...
                                    age_group1 = all_diseases_of_a_bucket_country_and_age_and_date(tempo->bucket_node_array[z], BUCKETSIZE, message, 0, 20, datetemp);
                                    age_group2 = all_diseases_of_a_bucket_country_and_age_and_date(tempo->bucket_node_array[z], BUCKETSIZE, message, 21, 40, datetemp);
                                    age_group3 = all_diseases_of_a_bucket_country_and_age_and_date(tempo->bucket_node_array[z], BUCKETSIZE, message, 41, 60, datetemp);
                                    age_group4 = all_diseases_of_a_bucket_country_and_age_and_date(tempo->bucket_node_array[z], BUCKETSIZE, message, 61, 500, datetemp);
...
/* Write the summary stats to the pipes */
//CHECK README FILE GIA TO PRWTOKOLLO EPIKOINWNIAS

                    //Create the pipes

                    //Generate the diseaseAggregator to Worker pipename
                    sprintf(pipename, "worker2agg%d", workernumber);

                    worker2agg_fd = open(pipename,  O_WRONLY);//I want it to block and wait for the other side of the pipe to open

                    //printf("\n%s\n", tempstring);
                    sleep(1);

                    write_message(worker2agg_fd, tempstring, buffersize);
                    //printf("eimai o %d kai egrapsa tosa %s", workernumber, date[j]);
                    free(introduction);
                    free(tempstring);
        ...
        //telos if den exw query
        }else{//An to msg prokeitai gia query
...
/*  diseaseFrequency  */
             if(!strcmp(token, "/diseaseFrequency")){
                ...
                if(count==2){
                    ...
                    if( (check_if_bucketnode_with_this_name_exists(virusname, diseasehash, disease_hash_position, BUCKETSIZE)==0) ){//Check if this virus exists
                        ...
                        write_message(worker2agg_fd, krousmata, buffersize);
                ...
                while(token != NULL){
                    ...
                    if(count==1){
                        rec_id = strdup(token);//get the record ID
                    }
                ...
                if( (check_if_bucketnode_with_this_name_exists( rec_id, recordhash, hash_number, BUCKETSIZE)!=1) ){//If there is NOT such a recordID in the record hash table
                    ...
                    write_message(worker2agg_fd, "NO", buffersize);
                ...
                write_message(worker2agg_fd, result, buffersize);
                ...
                while(token != NULL){
                    ...
                    if(count==1){
                        diseasename = strdup(token);//get the disease name
                    }else if(count==2){
                        date1 = strdup(token);
                    }else if(count==3){
                        date2 = strdup(token);
                    }else if(count==4){
                        countryn = strdup(token);
                    }
                ...
                if(count==2){
                    ...
                    write_message(worker2agg_fd, final_result, buffersize);
                ...
                while(token != NULL){
                    ...
                    if(count==1){
                        k = atoi(token);//get the k number
                    }else if(count==2){
                        countryn = strdup(token);
                    }else if(count==3){
                        diseasename = strdup(token);
                    }else if(count==4){
                        date1 = strdup(token);
                    }else if(count==5){
                        date2 = strdup(token);
                ...
                if(count!=6){//Wrong number of arguments :(
                    ...
                    for(x=0 ; x<k ; x++){
                        ...
                        if(a==0) sprintf(this_result, "41-60:  %lf%c", percentage, '%');
                        if(a==0) sprintf(this_result, "61+:  %lf%c", percentage, '%');
                    ...
                    while(temp!=NULL){
                        ...
                        for(k=0 ; k<(BUCKETSIZE/(sizeof(Bucket_node))) ; k++){
                            ...
                            if( temp->bucket_node_array[k] != NULL){
                                ...
                                char* this_result;
                                this_result = (char*)malloc( (strlen(temp->bucket_node_array[k]->name)+1+5/*pid*/+1)*sizeof(char) );
                                sprintf(this_result, "%s %lu", temp->bucket_node_array[k]->name, (long)getpid() );
            ...
            }else{//An dex8eis opoiadhpote allh entolh
                free(msg);
                if(workernumber==0) printf("Please check your query.Something's wrong :(\n");
                continue;
            }
/*--------------------*/
        }//if prokeitai gia query

        free(msg);

    }//if den exw provlhma me thn read moy
  ...
  if(signalq==1){
      char file_name[25];
      sprintf( file_name, "log_file.%lu", (long)getpid() );

      int fp;
      fp = open( file_name, O_RDWR | O_CREAT, 0644 );

      char* finres;
      Bucket_ptr proswrinos;

      int firstflag=0;

      for(i=0 ; i<COUNTRYHASHENTRIES ; i++){
          proswrinos = countryhash[i];


          int k=0;
          while(proswrinos!=NULL){

              for(k=0 ; k<(BUCKETSIZE/(sizeof(Bucket_node))) ; k++){

                  if(proswrinos->bucket_node_array[k]!=NULL){
                      char* this_one;
                      this_one = (char*)malloc( (strlen(proswrinos->bucket_node_array[k]->name)+1+5+1)*sizeof(char) );
                      sprintf(this_one, "%s", proswrinos->bucket_node_array[k]->name);

                      if(firstflag==0){
                          finres = strdup(this_one);
                          firstflag=1;
                          free(this_one);
                      }else{
                          char* prosstr;
                          prosstr = strdup(finres);

                          free(finres);

                          finres = (char*)malloc(  (strlen(prosstr)+1+strlen(this_one)+1)*sizeof(char) );
                          sprintf(finres, "%s\n%s", prosstr, this_one);
      ...
      char* recstats;
      recstats = (char*)malloc( (strlen(finres)+1+50+1)*sizeof(char) );

      sprintf(recstats, "%s\nTOTAL %d \nSUCCESS %d \nFAIL %d \n", finres, total_records, (total_records-failed_records), failed_records);

      free(finres);

      write(fp, recstats, strlen(recstats));
